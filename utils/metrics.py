...
class Evaluator:
  ...
  def average_precision_clevr(self, pred, attributes, distance_threshold):
    ...
    return self.compute_average_precision(
      np.array(precision_array, dtype=np.float32),
      np.array(recall_array, dtype=np.float32))

  def adjusted_rand_index(self, true_mask, pred_mask):
  ...
  def ari(self, pred_mask, true_mask, skip_0=False):
    ...
    for bi in range(B):
      ...
      if skip_0:
        p = p[t > 0]
        t = t[t > 0]
      ari_score = self.adjusted_rand_score(t, p)
